...
<table class="table" align="center" style="display: table;
  margin: auto;margin-right: 80px;width:80%;height=20px;border: 2px solid white;background-color:black;opacity:80%">
      <thead>
    <tr>
  ...
  <tbody>
    ...
    <tr v-for="item in items" :key="item.id" >
        ...
        <router-link :to="{name: 'EditCar' , params:{id:item.id}}" style="color:black"> <v-btn variant="flat" color="white"><i class="bi bi-pencil-square"></i>Edit</v-btn></router-link>
...
export default {
     ...
     methods:{
        ...
        async getCars(){
             try {
                 const response = await axios.get('http://127.0.0.1:8000/api/al');
