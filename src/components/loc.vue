...
<br><br><br>
<h3 class="cr">Liste des réservations</h3>
<br>
...
<table class="table" align="center" style="display: table;
  ...
  <tbody>
    <tr v-for="item in items" :key="item.id">
        ...
        <router-link :to="{name: 'facture'}"  style="color:white"> <v-btn variant="flat" color="white"><i class="bi bi-pencil-square"></i>Edit</v-btn></router-link>
        </td>
      </tr>
  </tbody>
</table>
...
export default {
     ...
     methods:{
        async getLocations(){
             try {
                 const response = await axios.get('http://127.0.0.1:8000/api/loc');
