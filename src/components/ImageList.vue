...
<br><br><br>
<h3 class="cr">Liste des images</h3>
<br>
<router-link :to="{name: 'UploadPic'}" class="custom-link" style="color:white">  <v-row class="my-custom-class" justify="center"><button class="main-btn"> Ajouter image <i class="fas fa-shopping-basket ps-3"></i></button></v-row></router-link>

<br><br>
<table class="table" align="center" style="display: table;margin: auto;margin-right: 80px;width:80%;height=20px;border: 2px solid white;background-color:black;opacity:80%">
  <thead>
    <tr>
  ...
  <tbody>
    ...
    <tr v-for="item in items" :key="item.id" >
        ...
        <td style="width:70%;color:white"><img :src="`http://localhost:8000/storage/images/${item.path}`" class="lp" style="width:140px"></td>
        <td >
        <button type="button" class="btn" @click="deletePic(item.id)"> <v-btn variant="flat" color="error"> <i class="bi bi-trash3-fill"></i>Delete</v-btn></button>
        <router-link :to="{name: 'EditImage' , params:{id:item.id}}" style="color:white"> <v-btn variant="flat" color="white"><i class="bi bi-pencil-square"></i>Edit</v-btn></router-link>
...
<div class="text-center">
   ...
   <i class="bi bi-chevron-left" style="color:red;font-weight:600;font-size:19px"></i><div class="btn-group me-2" role="group" aria-label="First group">
       <router-link :to="{name:'ImageList'}" style="color:black"><v-btn>1</v-btn></router-link>
   </div>
   <div class="btn-group me-2" role="group" aria-label="Second group">
       <router-link :to="{name:'ImaShw'}" style="color:black"><v-btn>2</v-btn></router-link>
   </div>
   <div class="btn-group" role="group" aria-label="Third group">
       <router-link :to="{name:'ImShow'}" style="color:black"><v-btn>3</v-btn></router-link>
   </div>
   <div class="btn-group" role="group" aria-label="foorth group" style=" margin: 10px;">
       <router-link :to="{name:'pic4'}" style="color:black"><v-btn>4</v-btn></router-link>
...
export default {
     ...
     methods:{
        ...
        async getPics(){
             try {
                 const response = await axios.get('http://127.0.0.1:8000/api/allPic');
         ...
         async deletePic(id){
              try {
                await axios.delete('http://127.0.0.1:8000/api/idelete/'+id);
                  this.getPics();
              } catch (error) {
                   console.log(error);
              }
