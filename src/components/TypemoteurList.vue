...
<br><br><br>
<h3 class="cr">Liste des types de moteurs</h3>
<br>

<router-link :to="{name: 'AddTypeMoteur'}" class="custom-link" style="color:white">  <v-row class="my-custom-class" justify="center"><button class="main-btn"> Ajouter type de moteur <i class="fas fa-shopping-basket ps-3"></i></button></v-row></router-link>

<br><br>
<table class="table" align="center" style="display: table;
  margin: auto;width:70%;height=20px;border: 2px solid white;background-color:black;opacity:80%">
  <thead>
    <tr>
  ...
  <tbody>
    <tr v-for="item in items" :key="item.id">
        ...
        <td>
        <button type="button" class="btn" @click="deleteTypemoteur(item.id)"> <v-btn variant="flat" color="error" ><i class="bi bi-trash3-fill"></i>Delete</v-btn></button>
        <router-link :to="{name: 'EditTypeMoteur' , params:{id:item.id}}"  style="color:white"> <v-btn variant="flat" color="white"><i class="bi bi-pencil-square"></i>Edit</v-btn></router-link>
        </td>
      </tr>
  </tbody>
</table>
